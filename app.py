...
.tx-row {
    ...
    border-radius:12px; margin-bottom:8px; transition: background .15s;
}
...
# ── DB helpers ────────────────────────────────────────────────────────────────
def db_lancamentos():
    return (supabase.table("lancamentos").select("*")
            .eq("user_id", uid()).order("data", desc=True).execute().data or [])
...
def db_investimentos():
    return (supabase.table("investimentos").select("*")
...
def db_metas():
    return (supabase.table("metas").select("*")
            .eq("user_id", uid()).execute().data or [])
...
with tab_dash:
        ...
        for t in txs[:6]:
            ...
            <div class="tx-row">
              ...
              <div style="flex:1">
                ...
                <div style="font-size:11px;opacity:.5;margin-top:2px">{t['categoria']} · {str(t['data'])[:10]}</div>
              </div>
    ...
    with col_mv:
        ...
        st.markdown(f"""
        <div class="ai-box">
          <div style="font-size:10px;letter-spacing:1.5px;opacity:.5;text-transform:uppercase;margin-bottom:8px">● IA Insight</div>
