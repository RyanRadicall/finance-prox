...
def login_screen():
    ...
    with col:
        ...
        if aba == "Entrar":
            ...
            nome_reg = st.text_input("Seu nome", placeholder="Como quer ser chamado?", key="auth_nome")
            if st.button("✨ Criar minha conta", use_container_width=True):
                try:
                    res = supabase.auth.sign_up({
                        "email": email, "password": senha,
                        "options": {"data": {"nome": nome_reg}}
                    })
                    st.success("Conta criada! Verifique seu e-mail e depois faça login.")
                except Exception as e:
